...
def run_for_sku(sku, d_sku, params_row, outdir):
    ...
    probe = [m for m in PROBE_METHODS if (m=="Intermittent") or (m in EXOG_METHODS_ENABLED_GLOBAL or m in ["ML-Exog RF","ML-Exog XGB"])]
    ...
    if need_escalate:
        ...
        if select_intermittent(d) and not should_dense_override(d):
            ...
            if HAVE_XGB: extra += ESCALATE_METHODS_DENSE
